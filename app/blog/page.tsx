...
const stories = [
  { slug: "fresh-way-to-shop-new-road", image: "/images/product-fashion.png", category: "Style guide", title: "A fresh way to shop New Road", excerpt: "From everyday essentials to a statement piece, build a shopping list that leaves space for an unexpected find." },
  { slug: "step-into-the-new-season", image: "/images/product-footwear.png", category: "New arrivals", title: "Step into the new season", excerpt: "Footwear is where comfort and personal style meet. Explore the shapes and finishes worth watching this season." },
  ...
  return <div className="view active blog-page" id="view-blog">
    ...
    <section className="section" id="latest">
      ...
      <div className="blog-grid">
        {stories.map((story, index) => <article className={`blog-card blog-card-${index + 1}`} key={story.slug}><div className="blog-image"><img src={story.image} alt="" loading={index < 3 ? "eager" : "lazy"} /></div><div className="blog-copy"><p>{story.category}</p><h2>{story.title}</h2><span>{story.excerpt}</span><Link href={`/blog/${story.slug}/`}>Read story <SiteIcon name="arrow" className="inline-icon" /></Link></div></article>)}
      </div>
    </section>
